# -*- coding: utf-8 -*-
"""F30k_CustomVIT_ATT_GRU (4).ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1Wab97QIiNiCSXYRobvR3aJNQtEpGLEC-
"""
...
path_train,  caption_train = all_img_vectort, cap_vector[:25000]
path_val, caption_val = all_img_vectorv, cap_vector[25000:]
...
**__________________________________---Custom VIT**
...
image_size = 256
patch_size = 16
num_patches = (image_size // patch_size) ** 2  # Now 196 for 224x224 images
...
data_augmentation = keras.Sequential(
    [
        ...
        layers.Resizing(image_size, image_size),  # Resize to match input shape
...
class Patches(layers.Layer):
    ...
    def call(self, images):
        ...
        # Use TensorFlow's extract_patches
...
class PatchEncoder(layers.Layer):
    ...
    def call(self, patch):
        # Generate positions for position embeddings
...
# Create your custom ViT model
def create_vit_feature_extractor():
    input_shape = (256,256,3)
    inputs = keras.Input(shape=input_shape)  # Input shape (32, 32, 3)
    # Augment data
   ...
   # patches = Patches(patch_size)(augmented)
    ...
    # Use layer normalization and flatten the output to get image features
    representation = layers.LayerNormalization(epsilon=1e-6)(encoded_patches)
    # representation = layers.Flatten()(representation)

    # Return the image feature vector
    return keras.Model(inputs=inputs, outputs=representation)
...
# Custom ViT feature extraction model
vit_feature_extractor = create_vit_feature_extractor()

# Freeze the layers of the ViT model
for layer in vit_feature_extractor.layers:
...
for batch in tqdm(New_Img):
    images, image_paths = batch  # Unpack the batch
    batch_features = vit_feature_extractor(images)  # Extract features

    for batch_feat, path in zip(batch_features, image_paths):
        feature_path = path.numpy().decode('utf-8')  # Convert path tensor to string
        img_features[feature_path] = batch_feat.numpy()  # Store features

# img_features now contains the image feature vectors
...
def evaluate(image):
    ...
    img_tensor_val = vit_feature_extractor(temp_input)
